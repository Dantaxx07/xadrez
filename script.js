...
function makeMove(fromRow, fromCol, toRow, toCol) {
    ...
    if (piece.type === PIECE_TYPES.PAWN && Math.abs(fromRow - toRow) === 2) {
        window.lastMove = move; // Marca o peão como elegível para en passant
    } else {
        window.lastMove = null; // Reseta se não for um movimento de peão duplo
    }

    if (validMoves.some(m => m.row === toRow && m.col === toCol && m.enPassant)) {
        const pawnDirection = piece.color === COLORS.WHITE ? 1 : -1;
        newBoard[toRow + pawnDirection][toCol] = null; // Remove o peão capturado por en passant
        move.capturedPiece = gameState.board[toRow + pawnDirection][toCol]; // Armazena a peça capturada
...
function handleUndo() {
    ...
    if (lastMove.castle) {
        ...
        newBoard[rookRow][rookFromCol] = null;
        rook.position = { row: rookRow, col: rookToCol };
        rook.hasMoved = false; // O roque é o primeiro movimento da torre
    }
