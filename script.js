...
function makeMove(fromRow, fromCol, toRow, toCol) {
    ...
    const selectedMove = gameState.validMoves.find(m => m.row === toRow && m.col === toCol);

    const move = {
        from: { row: fromRow, col: fromCol },
        to: { row: toRow, col: toCol },
        piece: piece.copy ? piece.copy() : { ...piece },
        capturedPiece: capturedPiece ? (capturedPiece.copy ? capturedPiece.copy() : { ...capturedPiece }) : null,
        pieceHadMoved: piece.hasMoved,
        isEnPassant: false,
        castle: selectedMove?.castle || false,
        castleType: selectedMove?.castleType || null
    };

    if (piece.type === PIECE_TYPES.PAWN && Math.abs(fromRow - toRow) === 2) {
        window.lastMove = move;
    } else {
        window.lastMove = null;
    }

    if (gameState.validMoves.some(m => m.row === toRow && m.col === toCol && m.enPassant)) {
        const pawnDirection = piece.color === COLORS.WHITE ? 1 : -1;
        newBoard[toRow + pawnDirection][toCol] = null;
        move.capturedPiece = gameState.board[toRow + pawnDirection][toCol];
    ...
    if (move.castle) {
        ...
        newBoard[rookRow][rookToCol] = rook;
        newBoard[rookRow][rookFromCol] = null;
        rook.position = { row: rookRow, col: rookToCol };
        rook.hasMoved = true;
    }
...
function handleUndo() {
    ...
    if (lastMove.capturedPiece) {
        if (lastMove.isEnPassant) {
            const pawnDirection = lastMove.piece.color === COLORS.WHITE ? -1 : 1;
            newBoard[lastMove.to.row + pawnDirection][lastMove.to.col] = lastMove.capturedPiece;
        } else {
